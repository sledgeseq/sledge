/* NEON optimized implementation of various MSV, Viterbi, and Forward
 * routines: structures, declarations, and macros.
 *
 * ML, Fri Mar 12 10:26:04 2021
 */
...
typedef struct p7_oprofile_s {
  ...
  uint8x16_t **rbv;       /* match scores [x][q]: rm, rm[0] are allocated      */
  uint8x16_t **sbv;       /* match scores for ssvfilter                        */
  ...
  int16x8_t **rwv;    /* [x][q]: rw, rw[0] are allocated  [Kp][Q8]         */
  int16x8_t  *twv;    /* transition score blocks          [8*Q8]           */
  ...
  float32x4_t **rfv;         /* [x][q]:  rf, rf[0] are allocated [Kp][Q4]         */
  float32x4_t  *tfv;          /* transition probability blocks    [8*Q4]           */
